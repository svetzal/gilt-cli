...
Local-only CLI for managing personal finance ledgers. Run with `uv run finance <command>`.
...
```
finance categorize --txid abc12345 --category "Housing:Utilities" --write
```
...
```
finance category --add "NewCategory" --description "..." --write
```

To add a subcategory:
```
finance category --add "Existing:NewSub" --write
```

Categories must exist in `config/categories.yml` before use. Use `finance categories` to see all defined categories.
...
uv run finance --data-dir ~/finances ingest --write
uv run finance --data-dir ~/finances migrate-to-events --write
```
...
uv run finance ingest                  # Preview
uv run finance ingest --write          # Persist
uv run finance rebuild-projections     # Update projections
```
...
# Single transaction
uv run finance categorize -a MYBANK_CHQ --txid abc12345 -c "Groceries" --write
...
### Budget review
```bash
uv run finance budget                              # Current year
uv run finance budget --year 2025 --month 10       # Specific month
uv run finance report --year 2025 --write          # Generate .docx
```

### Handle duplicates
```bash
uv run finance duplicates                          # ML-based scan
uv run finance duplicates --interactive            # Train ML with feedback
uv run finance mark-duplicate -p abc12345 -d def67890 --write
```

### Manage categories
```bash
uv run finance categories                          # View all
uv run finance category --add "Travel:Flights" --write
uv run finance category --set-budget "Dining Out" --amount 500 --write
uv run finance recategorize --from "OldName" --to "NewName" --write
uv run finance diagnose-categories                 # Find orphaned categories
```
...
| `--data-dir` after command | `--data-dir` is a **top-level** option: `finance --data-dir PATH budget`, not `finance budget --data-dir PATH`. |
| Category doesn't exist | Add it first: `finance category --add "Cat:Sub" --write` |
...
| Workspace not initialized | Run `finance --data-dir PATH init` to create directories and starter config. |
| Missing projections DB | Run `finance migrate-to-events --write` or `finance rebuild-projections`. |
| Missing event store | Run `finance migrate-to-events --write` first. |
...
uv run finance --data-dir /path/to/my/finances budget

# Via environment variable
FINANCE_DATA=/path/to/my/finances uv run finance budget
```
...
Path resolution is centralized in `finance.workspace.Workspace`. All command modules and services accept a `workspace: Workspace` parameter instead of individual path arguments.
...
```python
from finance.workspace import Workspace
from finance.cli.command.init import run as init_workspace

...
```bash
# Single transaction
finance categorize --account RBC_CHQ --txid abc123 \
  --category "Housing:Utilities" --write

# Batch by description
finance categorize --desc-prefix "HYDRO ONE" \
  --category "Housing:Utilities" --yes --write

# Batch by pattern
finance categorize --pattern "PAYMENT.*HYDRO" \
  --category "Housing:Utilities" --yes --write
```
...
# Utilities
finance categorize --pattern "HYDRO ONE|BC HYDRO" \
  --category "Housing:Utilities" --yes --write

...
```bash
# 1. Preview
finance categorize --desc-prefix "HYDRO" --category "Housing:Utilities"

# 2. Review output
# ... check that it looks correct ...

# 3. Write
finance categorize --desc-prefix "HYDRO" --category "Housing:Utilities" --write
```
...
# Combined
finance ytd --account RBC_CHQ --year 2025 --limit 50
```
...
├──────────────┼────────────┼──────────────────┼──────────┤
│ 2025-11-16   │ RBC_CHQ    │ GROCERY STORE    │ -$45.67  │
│ 2025-11-15   │ RBC_CHQ    │ SALARY DEPOSIT   │ $3000.00 │
└──────────────┴────────────┴──────────────────┴──────────┘
```
...
finance ytd --account RBC_CHQ | grep -E '\$[0-9]{3,}'

# Export to CSV
finance ytd --account RBC_CHQ --format csv > transactions.csv
```
...
```bash
PermissionError: [Errno 13] Permission denied: 'data/accounts/RBC_CHQ.csv'
```

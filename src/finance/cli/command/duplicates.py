...
def run(
    ...
    for i, match in enumerate(filtered_matches, 1):
        ...
        if interactive and detector.prompt_manager:
            try:
                user_says_duplicate = Confirm.ask(
                    "[yellow]Is this actually a duplicate?[/yellow]",
                    default=assessment.is_duplicate,
                )
                ...
                detector.prompt_manager.add_feedback(
                    ...
                    user_confirmed=user_says_duplicate,
                    llm_reasoning=assessment.reasoning,
                )

                feedback_count += 1
                if user_says_duplicate:
                    confirmed_count += 1

                # Show feedback confirmation
                if user_says_duplicate == assessment.is_duplicate:
                    console.print("[green]✓ Feedback recorded (LLM was correct)[/green]")
                else:
                    console.print("[yellow]✓ Feedback recorded (LLM will learn from this)[/yellow]")
                console.print()
    ...
    console.print(f"  LLM predicted duplicates: {duplicate_count}")
    console.print(f"  LLM predicted not duplicates: {len(filtered_matches) - duplicate_count}")

    if interactive and detector.prompt_manager and feedback_count > 0:
        console.print()
        console.print(f"[cyan]Interactive Session:[/cyan]")
        console.print(f"  Reviewed: {feedback_count}")
        console.print(f"  Confirmed as duplicates: {confirmed_count}")

        # Show overall stats
        stats = detector.prompt_manager.get_stats()
        if stats["total_feedback"] > 0:
            console.print()
            console.print(f"[cyan]Overall Learning Stats:[/cyan]")

        ...
        for ledger_path in sorted(data_dir.glob("*.csv")):
            try:
                ...
                for group in groups:
                    ...
                    d = t.date.isoformat() if hasattr(t.date, "isoformat") else str(t.date)
...
def run(
    *,
    ...
    spending = _aggregate_spending(data_dir, year, month)
    transactions_by_category = _collect_transactions(data_dir, year, month)

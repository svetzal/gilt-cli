...
from finance.ml.duplicate_classifier import DuplicateClassifier
...
def run(
    ...
    if not es_service.event_store_path.exists():
        ...
        console.print("Run 'finance migrate-to-events --write' first")
        return 1
...
def show_summary(console: Console, builder: TrainingDataBuilder) -> int:
    ...
    summary_table.add_row(
        ...
        "[green]Yes[/green]" if stats['sufficient_for_training'] else "[yellow]No (need 10+)[/yellow]"
    )

    console.print(summary_table)

    if stats['total_examples'] > 0:
        console.print(f"\n[dim]Use 'finance audit-ml --mode training' to see examples[/dim]")
        console.print(f"[dim]Use 'finance audit-ml --mode features' to see feature importance[/dim]")
        console.print(f"[dim]Use 'finance audit-ml --mode predictions' to test on current data[/dim]")
...
def show_training_data(
    ...
    if filter_pattern:
        ...
        filtered = [
            (p, l) for p, l in zip(pairs, labels)
    ...
    positive_pairs = [(p, l) for p, l in zip(pairs, labels) if l]
    if positive_pairs:
        console.print(f"[green]Positive Examples (Marked as Duplicates) - Showing {min(limit, len(positive_pairs))} of {len(positive_pairs)}[/green]\n")
    ...
    negative_pairs = [(p, l) for p, l in zip(pairs, labels) if not l]
    if negative_pairs:
...
def show_predictions(
    ...
    if filter_pattern:
        ...
        console.print(f"[dim]Filtered to {len(candidates)} candidates matching '{filter_pattern}'[/dim]\n")
    else:
    ...
    console.print(f"[cyan]ML Predictions - Showing {min(limit, len(candidates))} of {len(candidates)}[/cyan]\n")

...
class PromptManager:
    ...
    def _generate_learned_patterns(self) -> str:
        ...
        if false_positives:
            ...
            # Look for location pattern (e.g., Oshawa->Toronto vs Toronto->Oshawa)
            location_patterns = []
            for fp in false_positives[-5:]:  # Last 5 false positives
                desc1 = fp.get("txn1_description", "")
                desc2 = fp.get("txn2_description", "")
                if "Oshawa" in desc1 and "TORONTO" in desc2:
                    location_patterns.append("- Transit transactions with different locations (e.g., Oshawa vs Toronto) are typically separate trips, NOT duplicates")
                    break
                if "TORONTO" in desc1 and "Oshawa" in desc2:
                    location_patterns.append("- Transit transactions with different locations (e.g., Toronto vs Oshawa) are typically separate trips, NOT duplicates")
                    break

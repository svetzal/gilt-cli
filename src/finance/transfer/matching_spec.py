...
from finance.transfer.matching import compute_matches
...
def it_should_allow_scotia_curr_loc_same_sign_pair(tmp_path: Path):
    # Special-case: same-sign allowed for SCOTIA_CURR <-> SCOTIA_LOC
    sc_curr = tmp_path / "SCOTIA_CURR.csv"
    sc_loc = tmp_path / "SCOTIA_LOC.csv"
    ...
    assert len(matches) == 1
    m = matches[0]
    assert m.debit.account_id == "SCOTIA_CURR"
    assert m.credit.account_id == "SCOTIA_LOC"

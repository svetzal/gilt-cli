...
def _is_scotia_curr_loc_pair(a: str, b: str) -> bool:
    s = {a, b}
    return "SCOTIA_CURR" in s and "SCOTIA_LOC" in s


def _valid_sign_pair(d: Txn, o: Txn) -> bool:
    # Normally require opposite signs, but allow same-sign for special Scotia pair
    if d.amount * o.amount < 0:
        return True
    if _is_scotia_curr_loc_pair(d.account_id, o.account_id):
        return True
    return False
...
def _select_best_match(
    ...
    *,
    ...
    for c in candidates:
        ...
        if _is_scotia_curr_loc_pair(d.account_id, c.account_id):
            s = min(1.0, s + 0.05)

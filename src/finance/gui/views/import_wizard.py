...
from finance.gui.services.import_service import ImportService, ImportFileMapping, ImportResult
...
class ImportWorker(QThread):
    ...
    def __init__(self, service: ImportService, mappings: List[ImportFileMapping], write: bool):
        super().__init__()
...
class PreviewPage(QWizardPage):
    ...
    def _init_ui(self):
        ...
        warning.setStyleSheet("background-color: #fff3cd; padding: 8px; border-radius: 4px;")
...
class OptionsPage(QWizardPage):
    ...
    def _init_ui(self):
        ...
        # Info about dry-run
        dry_run_info = QLabel(
            "<i>Dry-run mode will validate files and show what would be imported "
            "without making any changes to your data.</i>"
        )
        dry_run_info.setWordWrap(True)
        dry_run_info.setStyleSheet("color: gray; padding-left: 20px;")
        ...
        duplicate_info.setStyleSheet("background-color: #d1ecf1; padding: 8px; border-radius: 4px;")
...
class ExecutePage(QWizardPage):
    ...
    def initializePage(self):
        ...
        # Start import
        self._start_import(wizard.service, mappings, write_enabled)

    def _start_import(self, service: ImportService, mappings: List[ImportFileMapping], write: bool):
        """Start the import operation in a background thread."""
        self.worker = ImportWorker(service, mappings, write)

...
class BudgetView(QWidget):
    ...
    def _init_ui(self):
        ...
        self.summary_label.setStyleSheet(
            "padding: 10px; border-radius: 4px; border: 1px solid palette(mid);"
        )
    ...
    def _load_budget(self):
        ...
        for item in summary.items:
            ...
            if item.is_category_header:
                ...
                # Blend base with a small amount of highlight for subtle effect
                blend_factor = 0.15
                r = int(base_color.red() * (1 - blend_factor) + highlight_color.red() * blend_factor)
                g = int(base_color.green() * (1 - blend_factor) + highlight_color.green() * blend_factor)
                b = int(base_color.blue() * (1 - blend_factor) + highlight_color.blue() * blend_factor)

                cat_item.setBackground(QColor(r, g, b))
            ...
            if item.remaining is not None:
                if item.remaining >= 0:
                    ...
                    remaining_color = QColor(39, 174, 96)  # Green
                else:
                    remaining_text = f"-${abs(item.remaining):,.2f}"
                    remaining_color = QColor(231, 76, 60)  # Red
            ...
            if item.percent_used is not None:
                ...
                if item.percent_used > 100:
                    pct_color = QColor(231, 76, 60)  # Red
                    font = pct_item.font()
                    font.setBold(True)
                    pct_item.setFont(font)
                elif item.percent_used > 90:
                    pct_color = QColor(243, 156, 18)  # Yellow/orange
                else:
                    pct_color = QColor(39, 174, 96)  # Green
    ...
    def _update_summary(self, summary):
        ...
        if summary.total_remaining >= 0:
            remaining_text = f'<span style="color: green;">${summary.total_remaining:,.2f}</span>'
            remaining_label = "Remaining"
        else:
            remaining_text = f'<span style="color: red;">${abs(summary.total_remaining):,.2f}</span>'
        ...
        if summary.over_budget_count > 0:
            ...
            summary_html += f'<p style="color: orange; margin-top: 8px;">⚠ {summary.over_budget_count} categor{plural} over budget</p>'

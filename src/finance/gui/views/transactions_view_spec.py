...
from finance.gui.views.transactions_view import IntelligenceWorker
from finance.model.account import Transaction, TransactionGroup
from finance.model.duplicate import DuplicateMatch, TransactionPair, DuplicateAssessment
...
class DescribeIntelligenceWorker:
    def it_should_scan_for_duplicates_and_categories(self):
        ...
        pair = TransactionPair(
            txn1_id="t1", txn1_date=date(2023, 1, 1), txn1_description="Test 1", txn1_amount=10.0, txn1_account="acc1",
            txn2_id="t2", txn2_date=date(2023, 1, 1), txn2_description="Test 2", txn2_amount=10.0, txn2_account="acc2"
        )
        assessment = DuplicateAssessment(is_duplicate=True, confidence=0.9, reasoning="High confidence")

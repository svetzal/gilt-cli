...
class DescribeShowSingleMatch:
    ...
    def it_should_return_zero_and_display_all_fields(self, tmp_path):
        workspace = Workspace(root=tmp_path)

        groups = [
            make_group(date="2025-03-15", description="EXAMPLE UTILITY PAYMENT", amount=-142.50)
        ]
    ...
    def it_should_display_placeholder_for_null_enrichment_fields(self, tmp_path):
        workspace = Workspace(root=tmp_path)
    ...
    def it_should_display_enrichment_fields_when_present(self, tmp_path):
        workspace = Workspace(root=tmp_path)
    ...
    def it_should_display_description_history_as_bulleted_list(self, tmp_path):
        workspace = Workspace(root=tmp_path)

        # Build projections manually with a multi-entry description_history
        groups = [make_group(date="2025-05-01", description="CURRENT DESCRIPTION", amount=-10.00)]
        _build_projections(workspace, groups)
        ...
        conn.execute(
            "UPDATE transaction_projections SET description_history = ? WHERE transaction_id = ?",
            (
    ...
    def it_should_display_placeholder_for_null_description_history(self, tmp_path):
        workspace = Workspace(root=tmp_path)

        groups = [make_group(date="2025-05-01", description="SINGLE DESCRIPTION", amount=-10.00)]
        _build_projections(workspace, groups)
...
class DescribeShowAmbiguousPrefix:
    ...
    def it_should_return_nonzero_and_list_candidates(self, tmp_path):
        workspace = Workspace(root=tmp_path)
...
class DescribeShowNoMatch:
    ...
    def it_should_return_one_and_print_clear_error(self, tmp_path):
        workspace = Workspace(root=tmp_path)

        groups = [make_group(date="2025-01-01", description="SOME TRANSACTION", amount=-5.00)]
        _build_projections(workspace, groups)
...
class DescribeShowPrefixValidation:
    ...
    def it_should_return_two_when_prefix_too_short(self, tmp_path):
        workspace = Workspace(root=tmp_path)
    ...
    def it_should_accept_full_16_char_transaction_id(self, tmp_path):
        workspace = Workspace(root=tmp_path)
...
class DescribeShowNoProjections:
    ...
    def it_should_return_one_when_projections_db_missing(self, tmp_path):
        workspace = Workspace(root=tmp_path)

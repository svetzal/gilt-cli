...
class DescribeSkillInit:
    def it_should_create_skill_files_in_target_directory(self, tmp_path):
        target = tmp_path
        with (
    ...
    def it_should_stamp_version_into_skill_md(self, tmp_path):
        target = tmp_path
        with (
            patch("gilt.cli.command.skill_init._get_package_version", return_value="0.4.1"),
            patch("gilt.cli.command.skill_init.Path.cwd", return_value=target),
    ...
    def it_should_not_stamp_version_into_command_reference(self, tmp_path):
        target = tmp_path
        with (
            patch("gilt.cli.command.skill_init._get_package_version", return_value="0.4.1"),
            patch("gilt.cli.command.skill_init.Path.cwd", return_value=target),
    ...
    def it_should_install_to_global_directory_with_flag(self, tmp_path):
    ...
    def it_should_refuse_to_overwrite_when_installed_version_is_newer(self, tmp_path):
        target = tmp_path
    ...
    def it_should_overwrite_newer_version_with_force(self, tmp_path):
        target = tmp_path
        skill_dir = target / ".claude" / "skills" / "gilt"
        skill_dir.mkdir(parents=True)
        skill_md = skill_dir / "SKILL.md"
    ...
    def it_should_always_install_when_no_version_in_existing_file(self, tmp_path):
        target = tmp_path
        skill_dir = target / ".claude" / "skills" / "gilt"
        skill_dir.mkdir(parents=True)
        skill_md = skill_dir / "SKILL.md"
    ...
    def it_should_report_up_to_date_when_content_unchanged(self, tmp_path):
        target = tmp_path
        # First install
        with (
            patch("gilt.cli.command.skill_init._get_package_version", return_value="0.4.1"),
    ...
    def it_should_return_1_when_any_file_is_skipped(self, tmp_path):
...
class DescribeJsonOutput:
    def it_should_emit_json_with_expected_top_level_keys(self, tmp_path, capsys):
        target = tmp_path
        with (
            patch("gilt.cli.command.skill_init._get_package_version", return_value="0.4.1"),
            patch("gilt.cli.command.skill_init.Path.cwd", return_value=target),
    ...
    def it_should_report_success_true_on_clean_install(self, tmp_path, capsys):
        target = tmp_path
        with (
            patch("gilt.cli.command.skill_init._get_package_version", return_value="0.4.1"),
            patch("gilt.cli.command.skill_init.Path.cwd", return_value=target),
    ...
    def it_should_report_success_false_when_any_file_skipped(self, tmp_path, capsys):
        target = tmp_path
        skill_dir = target / ".claude" / "skills" / "gilt"
        skill_dir.mkdir(parents=True)
        skill_md = skill_dir / "SKILL.md"
    ...
    def it_should_report_up_to_date_as_success(self, tmp_path, capsys):
        target = tmp_path
    ...
    def it_should_not_emit_rich_markup_in_json_mode(self, tmp_path, capsys):
        target = tmp_path
        with (
            patch("gilt.cli.command.skill_init._get_package_version", return_value="0.4.1"),
            patch("gilt.cli.command.skill_init.Path.cwd", return_value=target),

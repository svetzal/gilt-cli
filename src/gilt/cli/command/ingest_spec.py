...
class DescribeLoadLedgerCounts:
    ...
    def it_should_return_zero_count_for_unreadable_file(self, tmp_path):
...
class DescribeIngestRunDryRun:
    ...
    def it_should_return_zero_with_empty_ingest_dir(self, tmp_path):
    ...
    def it_should_not_create_any_output_files_in_dry_run(self, tmp_path):
        ws = Workspace(root=tmp_path)
        ws.ingest_dir.mkdir(parents=True, exist_ok=True)

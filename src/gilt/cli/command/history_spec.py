...
class DescribeHistoryBasicGrouping:
    ...
    def it_should_group_by_category_and_subcategory(self, tmp_path):
...
class DescribeHistoryCaseInsensitivity:
    ...
    def it_should_match_pattern_regardless_of_case(self, tmp_path):
        workspace = Workspace(root=tmp_path)
...
class DescribeHistoryAccountFilter:
    ...
    def it_should_restrict_to_one_account_when_account_flag_set(self, tmp_path):
        workspace = Workspace(root=tmp_path)
...
class DescribeHistoryIncludeUncategorized:
    ...
    def it_should_show_uncategorized_row_when_flag_set(self, tmp_path):
        workspace = Workspace(root=tmp_path)
...
class DescribeHistoryLimit:
    ...
    def it_should_truncate_results_to_limit(self, tmp_path):
        workspace = Workspace(root=tmp_path)

        # Five distinct categories
        cats = ["CatA", "CatB", "CatC", "CatD", "CatE"]
        txn_ids = [f"{i:016x}" for i in range(len(cats))]
        groups = [_make_group(_make_txn(txn_id, "ACME CORP INVOICE", -10.00)) for txn_id in txn_ids]
        _build_projections(workspace, groups)
...
class DescribeHistoryOrdering:
    ...
    def it_should_order_by_count_descending(self, tmp_path):
        workspace = Workspace(root=tmp_path)
        ...
        groups = [
            _make_group(_make_txn(txn_id, "SAMPLE STORE PURCHASE", -5.00)) for txn_id, _ in entries
        ]
        _build_projections(workspace, groups)
...
class DescribeHistoryEmptyResult:
    ...
    def it_should_return_zero_and_print_no_match_message(self, tmp_path):
        workspace = Workspace(root=tmp_path)
...
class DescribeHistoryDateWindow:
    ...
    def it_should_filter_to_date_range(self, tmp_path):
        workspace = Workspace(root=tmp_path)
...
class DescribeHistoryDuplicatesExcluded:
    ...
    def it_should_exclude_marked_duplicates(self, tmp_path):
        workspace = Workspace(root=tmp_path)
        import sqlite3

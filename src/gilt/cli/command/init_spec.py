...
from gilt.cli.command.init import run
...
class DescribeInitCommand:
    ...
    def it_should_create_all_directories_and_config_files(self, tmp_path):
    ...
    def it_should_be_safe_to_run_twice(self, tmp_path):
        workspace = Workspace(root=tmp_path)
    ...
    def it_should_skip_existing_directories(self, tmp_path):
        workspace = Workspace(root=tmp_path)
    ...
    def it_should_create_valid_starter_categories_config(self, tmp_path):
        workspace = Workspace(root=tmp_path)
    ...
    def it_should_create_parseable_accounts_config(self, tmp_path):
        workspace = Workspace(root=tmp_path)
        run(workspace=workspace)

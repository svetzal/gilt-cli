...
class DescribeGetSpendingByCategory:
    ...
    def it_should_return_empty_dict_when_no_ledger_files_exist(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
    ...
    def it_should_aggregate_negative_amounts_as_spending(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
    ...
    def it_should_ignore_positive_amounts(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
    ...
    def it_should_filter_by_year_and_month(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
    ...
    def it_should_skip_uncategorized_transactions(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
...
class DescribeGetTotalSpending:
    ...
    def it_should_sum_all_category_spending(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
    ...
    def it_should_return_zero_with_no_transactions(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
...
class DescribeGetUncategorizedCount:
    ...
    def it_should_count_transactions_with_no_category(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
    ...
    def it_should_return_zero_when_all_categorized(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
...
class DescribeGetBudgetSummary:
    ...
    def it_should_default_year_to_current_year_when_both_year_and_month_are_none(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
    ...
    def it_should_prorate_monthly_budget_to_yearly_report(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
    ...
    def it_should_prorate_yearly_budget_to_monthly_report(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
    ...
    def it_should_include_over_budget_categories_in_over_budget_count(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
    ...
    def it_should_not_count_categories_without_budget_in_total_budgeted(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
    ...
    def it_should_include_subcategory_items_in_budget_summary(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
    ...
    def it_should_filter_to_requested_category_only(self, tmp_path):
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
...
class DescribeBuildCategoryBudgetItems:
    ...
    def _make_service(self, tmp_path) -> BudgetService:
        data_dir = tmp_path / "data" / "accounts"
        data_dir.mkdir(parents=True)
        cats_path = tmp_path / "config" / "categories.yml"
        cats_path.parent.mkdir(parents=True)
    ...
    def it_should_return_header_plus_one_item_per_subcategory(self, tmp_path):
    ...
    def it_should_return_correct_budget_and_actual_for_totals(self, tmp_path):
        service = self._make_service(tmp_path)
        cat = Category(
            name="Shopping",
    ...
    def it_should_set_is_over_budget_true_when_spending_exceeds_budget(self, tmp_path):
        service = self._make_service(tmp_path)
        cat = Category(
            name="Dining",
    ...
    def it_should_return_zero_budget_for_totals_when_category_has_no_budget(self, tmp_path):
        service = self._make_service(tmp_path)

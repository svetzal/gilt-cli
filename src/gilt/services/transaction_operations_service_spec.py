...
class DescribeResolveTransactionTargets(DescribeTransactionOperationsService):
    ...
    def it_should_find_by_txid_prefix(self, service, sample_groups):
        result = service.find_transaction_targets(sample_groups, txid="abc123de")
    ...
    def it_should_return_error_when_txid_prefix_too_short(self, service, sample_groups):
        result = service.find_transaction_targets(sample_groups, txid="abc")

        assert isinstance(result, str)
        assert "at least 8 characters" in result

    def it_should_return_error_when_txid_not_found(self, service, sample_groups):
        result = service.find_transaction_targets(sample_groups, txid="zzz99999")
    ...
    def it_should_find_by_description_criteria(self, service, sample_groups):
        result = service.find_transaction_targets(sample_groups, description="NETFLIX SUBSCRIPTION")
    ...
    def it_should_find_by_desc_prefix_criteria(self, service, sample_groups):
        result = service.find_transaction_targets(sample_groups, desc_prefix="SPOTIFY")

        assert isinstance(result, list)
        assert len(result) == 2

    def it_should_find_by_pattern_criteria(self, service, sample_groups):
        result = service.find_transaction_targets(sample_groups, pattern=r"SPOTIFY|NETFLIX")

        assert isinstance(result, list)
        assert len(result) == 3

    def it_should_return_error_for_invalid_pattern(self, service, sample_groups):
        result = service.find_transaction_targets(sample_groups, pattern="[invalid(")
    ...
    def it_should_return_empty_list_when_no_criteria_matches(self, service, sample_groups):
        result = service.find_transaction_targets(sample_groups, description="NONEXISTENT")

        assert isinstance(result, list)
        assert len(result) == 0


class DescribeFindProjectionByPrefix:
    """Tests for TransactionOperationsService.find_projection_by_prefix."""
    ...
    def it_should_return_error_when_prefix_too_short(self, service, sample_txns):
        result = service.find_projection_by_prefix("abc123", sample_txns)
        assert result.transaction is None
        assert result.error == "prefix_too_short"

    def it_should_return_error_when_not_found(self, service, sample_txns):
        result = service.find_projection_by_prefix("notfound", sample_txns)
        assert result.transaction is None
    ...
    def it_should_return_error_when_ambiguous(self, service, sample_txns):
        ...
        result = service.find_projection_by_prefix("abc123de", sample_txns)
        assert result.transaction is None
    ...
    def it_should_return_transaction_on_exact_match(self, service, sample_txns):
        result = service.find_projection_by_prefix("abc123def456789a", sample_txns)
    ...
    def it_should_return_transaction_on_unique_prefix(self, service, sample_txns):
        ...
        result = service.find_projection_by_prefix("xyz789uv", sample_txns)
        assert result.error is None
        assert result.transaction is not None
    ...
    def it_should_cap_ambiguous_matches_at_five(self, service):
        ...
        result = service.find_projection_by_prefix("abc123de", txns)
        assert result.error == "ambiguous"

...
class DescribeEventSourcingServicePathResolution:
    ...
    def it_should_use_workspace_paths_when_workspace_given(self, tmp_path):
    ...
    def it_should_prefer_explicit_paths_over_workspace_paths(self, tmp_path):
        ws = Workspace(root=tmp_path)
        explicit_es = tmp_path / "custom_events.db"
        explicit_proj = tmp_path / "custom_projections.db"
...
class DescribeCheckEventStoreStatus:
    ...
    def it_should_report_not_exists_when_db_file_absent(self, tmp_path):
        ws = Workspace(root=tmp_path)
        service = EventSourcingService(workspace=ws)
    ...
    def it_should_report_exists_after_event_store_is_created(self, tmp_path):
        ws = Workspace(root=tmp_path)
    ...
    def it_should_count_csv_files_in_data_dir_when_event_store_missing(self, tmp_path):
        ws = Workspace(root=tmp_path)
    ...
    def it_should_not_count_csv_files_when_event_store_exists(self, tmp_path):
        ws = Workspace(root=tmp_path)
        ws.event_store_path.parent.mkdir(parents=True, exist_ok=True)
...
class DescribeCheckProjectionStatus:
    ...
    def it_should_report_not_exists_when_projections_db_absent(self, tmp_path):
        ws = Workspace(root=tmp_path)
        ws.event_store_path.parent.mkdir(parents=True, exist_ok=True)
        store = EventStore(str(ws.event_store_path))
    ...
    def it_should_report_up_to_date_when_projections_match_events(self, tmp_path):
        ws = Workspace(root=tmp_path)
        ws.event_store_path.parent.mkdir(parents=True, exist_ok=True)
        store = EventStore(str(ws.event_store_path))
    ...
    def it_should_report_outdated_when_new_events_added_after_build(self, tmp_path):
        ws = Workspace(root=tmp_path)
        ws.event_store_path.parent.mkdir(parents=True, exist_ok=True)
        store = EventStore(str(ws.event_store_path))
...
class DescribeGetEventStore:
    ...
    def it_should_return_an_event_store_instance(self, tmp_path):
        ws = Workspace(root=tmp_path)
        ws.event_store_path.parent.mkdir(parents=True, exist_ok=True)
        service = EventSourcingService(workspace=ws)
    ...
    def it_should_create_db_file_on_get(self, tmp_path):
        ws = Workspace(root=tmp_path)
        ws.event_store_path.parent.mkdir(parents=True, exist_ok=True)
        service = EventSourcingService(workspace=ws)
...
class DescribeGetProjectionBuilder:
    ...
    def it_should_return_a_projection_builder_instance(self, tmp_path):
        ws = Workspace(root=tmp_path)
...
class DescribeEnsureProjectionsUpToDate:
    ...
    def it_should_return_zero_when_projections_already_current(self, tmp_path):
        ws = Workspace(root=tmp_path)
        ws.event_store_path.parent.mkdir(parents=True, exist_ok=True)
        store = EventStore(str(ws.event_store_path))
    ...
    def it_should_return_positive_count_when_projections_are_rebuilt(self, tmp_path):
        ws = Workspace(root=tmp_path)
        ws.event_store_path.parent.mkdir(parents=True, exist_ok=True)
        store = EventStore(str(ws.event_store_path))
    ...
    def it_should_use_incremental_rebuild_when_projections_are_outdated(self, tmp_path):
        ws = Workspace(root=tmp_path)
        ws.event_store_path.parent.mkdir(parents=True, exist_ok=True)
        store = EventStore(str(ws.event_store_path))
...
class DescribeEnsureReady:
    ...
    def it_should_return_no_data_when_event_store_missing_and_no_csvs(self, tmp_path):
        ws = Workspace(root=tmp_path)
        service = EventSourcingService(workspace=ws)
    ...
    def it_should_return_no_event_store_when_csvs_exist_but_no_store(self, tmp_path):
        ws = Workspace(root=tmp_path)
    ...
    def it_should_return_ready_with_store_and_builder_when_all_present(self, tmp_path):
        ws = Workspace(root=tmp_path)
        ws.event_store_path.parent.mkdir(parents=True, exist_ok=True)
        store = EventStore(str(ws.event_store_path))
        _append_transaction_event(store, txn_id="aaaa0001aaaa0001", account="MYBANK_CHQ")
    ...
    def it_should_auto_rebuild_projections_when_missing(self, tmp_path):
        ws = Workspace(root=tmp_path)
        ws.event_store_path.parent.mkdir(parents=True, exist_ok=True)
        store = EventStore(str(ws.event_store_path))
        _append_transaction_event(store, txn_id="bbbb0001bbbb0001", account="MYBANK_CHQ")
    ...
    def it_should_auto_rebuild_projections_when_outdated(self, tmp_path):
        ws = Workspace(root=tmp_path)
        ws.event_store_path.parent.mkdir(parents=True, exist_ok=True)
        store = EventStore(str(ws.event_store_path))
        _append_transaction_event(store, txn_id="cccc0001cccc0001", account="MYBANK_CHQ")
